...
async def get_deadline_stats(
    ...
    # Получаем текущее время в UTC для сравнения с дедлайнами
    ...
    statement = select(
        func.sum(
            case(
                ((Task.completed == True) & (Task.completed_at <= Task.deadline_at), 1), else_=0
            )
        ...
        func.sum(
            case(
                ((Task.completed == False) & (Task.deadline_at != None) & (Task.deadline_at > now_utc), 1), else_=0
            )
